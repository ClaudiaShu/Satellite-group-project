...
def cal_a1(phi, omega, kappa):
    a1 = math.cos(phi) * math.cos(kappa) - math.sin(phi) * \
...
def cal_a2(phi, omega, kappa):
    a2 = -math.cos(phi) * math.sin(kappa) - math.sin(phi) * \
...
def cal_c1(phi, omega, kappa):
    c1 = math.sin(phi) * math.cos(kappa) + math.cos(phi) * \
        math.sin(omega) * math.sin(kappa)
    return c1
    # pass
...
def cal_c2(phi, omega, kappa):
    c2 = -math.sin(phi) * math.sin(kappa) + math.cos(phi) * \
        math.sin(omega) * math.cos(kappa)
    return c2
    # pass
...
def cal_a15(x, x0, y, y0, phi, omega, kappa, f):
    a15 = -f * math.sin(kappa) - (x - x0) * ((x - x0) *
                                             math.sin(kappa) + (y - y0) * math.cos(kappa)) / f
    return a15
    # pass
...
def cal_a25(x, x0, y, y0, phi, omega, kappa, f):
    a25 = -f * math.cos(kappa) - (y - y0) * ((x - x0) *
                                             math.sin(kappa) + (y - y0) * math.cos(kappa)) / f
    return a25
    # pass
...
def interpolation_q(data, time):
    ...
    q0 = np.array([data[front][1], data[front][2],
                   data[front][3], data[front][4]])
...
def InterpRmats(time, Rmats, timeCodes):
    ...
    q0q1 = float(np.dot(q0_, q1_))
    theta = math.acos(q0q1)
    miu0 = math.sin(theta * (timeCodes[i + 1] - time) /
                    (timeCodes[i + 1] - timeCodes[i])) / math.sin(theta)
    miu1 = math.sin(
        theta * (time - timeCodes[i]) / (timeCodes[i + 1] - timeCodes[i])) / math.sin(theta)
...
def XYZ2BLH(data):
    ...
    for i in range(0, num):
        ...
        while iteration == True:
            ...
            N = wgs84_a / W
            H = H + dH
            B = math.atan(Z * (N + H) / (pow(X * X + Y * Y, 0.5)
                                         * (N * (1 - wgs84_e2) + H)))
            L = math.atan(Y / X)
            if Y > 0:
                if L < 0:
    ...
    for li in range(hei):
        for lj in range(wid):
            ...
            for lk in range(layers):
                ...
                B = 2 * ((im_gps[0, 0] * u[0] + im_gps[0, 1]
                          * u[1]) / a / a + im_gps[0, 2] * u[2] / b / b)
                C = (im_gps[0, 0] * im_gps[0, 0] + im_gps[0, 1] * im_gps[0, 1]) / a / a + im_gps[0, 2] * im_gps[
                    0, 2] / b / b - 1
                uk = (-B - math.sqrt(B * B - 4 * A * C)) / 2 / A
                point_ = [im_gps[0, 0] + uk * u[0], im_gps[0, 1] + uk * u[1],
                          im_gps[0, 2] + uk * u[2], grid_x[li, lj], grid_y[li, lj]]
                data.append(point_)
    data = np.array(data)
    return data
